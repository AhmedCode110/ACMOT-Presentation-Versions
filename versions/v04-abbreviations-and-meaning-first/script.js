...
(function () {
  ...
  /* ------------------------------------------------ v3: on-slide "In simple words + Example" strip */
  var XI = window.EXPLAIN_INLINE || {};
  var XKIND = { toy: 'made-up numbers', real: 'real thesis numbers', pub: 'published numbers' };
  slides.forEach(function (s) {
    var d = XI[s.getAttribute('data-title')];
    if (!d || s.hasAttribute('data-nostrip')) return;
    var x = document.createElement('div'); x.className = 'xs';
    x.innerHTML = '<div class="xs-simple"><div class="xs-h">In simple words</div><p>' + d[1] + '</p></div>' +
      '<div class="xs-ex"><div class="xs-h">Example' + (XKIND[d[0]] ? '<span class="xs-kind ' + d[0] + '">' + XKIND[d[0]] + '</span>' : '') + '</div><p>' + d[2] + '</p></div>';
    var dv = $('.dv-text', s);
    if (dv) dv.appendChild(x); else s.appendChild(x);
  });
  bindAll(document);
  ...
  var ABBR = [
    ...
    ['U2MOT', 'Uncertainty-aware Unsupervised Multi-Object Tracking'],
  ...
  var ABBR_SKIP = 'svg,.code,.formula,.s-foot,.notes,.s-head,h1,.qmark,.dv-num,.tabs,.lbl,.kpi .v,.big,.cap,[data-bind],.abbr-x,.prov,.newtag,.adapt,.lock,#u2setup,.ol-card .n';
  ...
  function bars(h, o) {
    ...
    badge(s, w, o.better);
    var m = { l: o.ml || 66, r: 14, t: (o.title || o.better) ? (o.ref && !o.title ? 84 : 60) : 24, b: o.mb || 66 };
  ...
  function stepBars(h, key, o) {
    var r = D.devAblation.rows.slice(0, 4), vals = r.map(function (x) { return x[key]; });
    bars(h, { labels: ['A0\nBaseline', 'A1\nTuned', 'A2\nAdaptive conf.', 'A3\nAC-MOT'], values: vals,
      colors: [COL.a0, COL.a1, COL.a2, COL.a3], better: o.better, dec: o.dec, unit: o.unit, ylabel: o.ylabel, ref: o.ref, max: o.max, bw: 96, mb: 76 });
    var svg = h.querySelector('svg'), rects = svg.querySelectorAll('rect.bar');
    for (var i = 1; i < rects.length; i++) {
      var a = rects[i - 1], b = rects[i], d = vals[i] - vals[i - 1];
      var x = (+a.getAttribute('x') + +a.getAttribute('width') + +b.getAttribute('x')) / 2;
      var y = Math.min(+a.getAttribute('y'), +b.getAttribute('y')) - 42;
      var good = o.better === 'lower' ? d < 0 : d > 0;
      T(svg, x, y, (d > 0 ? '+' : '') + fm(d, o.dec), 'cval', { 'text-anchor': 'middle', fill: good ? '#15803D' : '#B91C1C', style: 'font-size:22px' });
    }
  }
  ...
  var CHARTS = {
    ...
    'fin-ids': function (h) {
      var H_ = D.historical;
      bars(h, { labels: ['Baseline', 'Full\nAC-MOT', 'MATCH\n_090'], values: [H_.baseline.ids, H_.full.ids, H_.match090.ids], colors: [COL.base, COL.full, COL.match], better: 'lower', dec: 0, ylabel: 'IDS', bw: 64, mb: 76 });
    },
    'fin-fps': function (h) {
      var H_ = D.historical;
      bars(h, { labels: ['Baseline', 'Full\nAC-MOT', 'MATCH\n_090'], values: [H_.baseline.fps, H_.full.fps, H_.match090.fps], colors: [COL.base, COL.full, COL.match], better: 'higher', dec: 1, ref: REF25, ylabel: 'FPS', bw: 64, mb: 76 });
    },
    'smooth': function (h) {
    ...
    'dev-mota-steps': function (h) { stepBars(h, 'mota', { better: 'higher', dec: 2, unit: '%', ylabel: 'MOTA (%)', max: 60 }); },
    'dev-recall-steps': function (h) { stepBars(h, 'recall', { better: 'higher', dec: 2, unit: '%', ylabel: 'Recall (%)', max: 62 }); },
    'dev-ids-steps': function (h) { stepBars(h, 'ids', { better: 'lower', dec: 0, ylabel: 'ID switches', max: 3500 }); },
    'dev-fps-steps': function (h) { stepBars(h, 'fps', { better: 'higher', dec: 2, ylabel: 'FPS', max: 40, ref: REF25 }); },
    'speed-u2': function (h) {
      var v1 = tdRow('v1'), v2 = tdRow('v2');
      bars(h, { labels: ['AC-MOT V1\nYOLOv8n', 'AC-MOT V2\nYOLOv8n', 'U2MOT\nYOLOX-X'], values: [v1.fps, v2.fps, 5.5], vtext: [null, null, '≈ 5–6'],
        colors: [COL.v1, COL.v2, COL.u2], better: 'higher', dec: 1, ref: REF25, ylabel: 'Frames per second (Tesla T4)', bw: 110, mb: 76, max: 50 });
    }
  };
