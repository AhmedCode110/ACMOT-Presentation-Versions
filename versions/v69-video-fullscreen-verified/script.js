...
(function () {
  ...
  slides.forEach(function (s, i) {
    ...
    var mv = s.getAttribute('data-video');
    if (mv) {
      var mp = mv.split('|'), vb = document.createElement('button');
      vb.type = 'button'; vb.className = 'vbtn'; vb.setAttribute('data-video-index', mp[0] || '0');
      vb.title = 'Open the AC-MOT and baseline video comparison';
      vb.innerHTML = '<span class="vi">▶</span>' + (mp[1] || 'Watch videos');
      f.appendChild(vb);
    }
  ...
  var ICON = {
    ...
    ,pictures: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round"><rect x="3" y="4" width="18" height="16" rx="2"/><circle cx="8.5" cy="9" r="1.5"/><path d="m4 17 5-5 3 3 2-2 6 6"/></svg>'
    ,videos: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round"><rect x="3" y="5" width="13" height="14" rx="2"/><path d="m16 10 5-3v10l-5-3z"/></svg>'
  };
  ...
  ui.innerHTML =
    ...
    '<div class="bar">' +
      ...
      '<button data-act="pictures" class="library-btn" title="Open picture library" aria-label="Picture library">' + ICON.pictures + '<span>Pictures</span></button>' +
      '<button data-act="videos" class="library-btn" title="Open video library" aria-label="Video library">' + ICON.videos + '<span>Videos</span></button>' +
  ...
  /* v64: videos are a separate library, not part of explanation slides. */
  var mview = document.getElementById('mview'), mediaItems = [
    ['Baseline vs adaptive · uav0000249', 'assets/videos/a0_vs_a3_uav0000249.mp4', 'A side-by-side comparison clip used in the AC-MOT result discussion.'],
    ['Tiny objects · uav0000188', 'assets/videos/a0_vs_a3_uav0000188.mp4', 'A comparison clip showing a difficult aerial sequence.'],
    ['Baseline vs final · uav0000249', 'assets/videos/acmot_baseline_vs_final_uav0000249.mp4', 'A baseline and final AC-MOT comparison clip.'],
    ['Aerial reference', 'assets/videos/aerial_reference.mp4', 'Reference aerial footage used to show the tracking setting.'],
    ['Highway reference', 'assets/videos/highway_reference.mp4', 'Reference highway footage used in the presentation.']
  ], mediaIndex = 0;
  function openMedia(i) {
    if (!mview) return;
    closeOverlays(); mediaIndex = Math.max(0, Math.min(mediaItems.length - 1, i || 0));
    var tabs = $('.mtabs', mview), player = $('.mplayer', mview), cap = $('.mcaption', mview);
    tabs.innerHTML = '';
    mediaItems.forEach(function (item, j) {
      var b = document.createElement('button'); b.type = 'button'; b.className = j === mediaIndex ? 'on' : '';
      b.textContent = item[0]; b.addEventListener('click', function () { openMedia(j); }); tabs.appendChild(b);
    });
    /* v69: swap only the <video>, so the full-screen button on the player survives.
       Lazy loading is kept: the file is requested only for the selected item. */
    var old = $('video', player);
    if (old) { old.pause(); old.removeAttribute('src'); old.load(); player.removeChild(old); }
    var v = document.createElement('video');
    v.setAttribute('controls', ''); v.setAttribute('playsinline', ''); v.setAttribute('preload', 'metadata');
    v.src = mediaItems[mediaIndex][1];
    player.insertBefore(v, player.firstChild);
    cap.textContent = mediaItems[mediaIndex][2]; mview.classList.add('open');
    var p = v.play(); if (p && p.catch) p.catch(function () {});
  }
  /* v69: full screen for the selected video, with a fallback for browsers and
     embedded frames where the Fullscreen API is missing or blocked by policy. */
  function fsElement() {
    return document.fullscreenElement || document.webkitFullscreenElement || document.msFullscreenElement || null;
  }
  function zoomOn() { return !!(mview && mview.classList.contains('mzoom')); }
  function setFullLabels() {
    if (!mview) return;
    var on = !!fsElement() || zoomOn();
    $$('.mfull', mview).forEach(function (b) {
      b.textContent = on ? 'Exit full screen' : 'Full screen video';
      b.setAttribute('aria-pressed', on ? 'true' : 'false');
    });
  }
  function zoomEnter(msg) {
    if (!mview) return;
    mview.classList.add('mzoom');
    var n = $('.mnote', mview);
    if (n) {
      n.textContent = msg || 'This browser did not allow full screen, so the video is shown at full window size instead. Press Esc or use the button to go back.';
      n.classList.add('on');
    }
    setFullLabels();
  }
  function zoomExit() {
    if (!mview) return;
    mview.classList.remove('mzoom');
    var n = $('.mnote', mview); if (n) { n.classList.remove('on'); n.textContent = ''; }
    setFullLabels();
  }
  function exitFull() {
    if (zoomOn()) { zoomExit(); return; }
    var fn = document.exitFullscreen || document.webkitExitFullscreen || document.msExitFullscreen;
    if (fn) { try { var p = fn.call(document); if (p && p['catch']) p['catch'](function () {}); } catch (e) {} }
  }
  /* Some embedded frames neither grant full screen nor reject the request — the promise
     simply never settles. This watchdog makes sure the button always does something. */
  var fsWatch = null;
  function clearWatch() { if (fsWatch) { clearTimeout(fsWatch); fsWatch = null; } }
  function armWatch() {
    clearWatch();
    fsWatch = setTimeout(function () { fsWatch = null; if (!fsElement() && !zoomOn()) zoomEnter(); }, 900);
  }
  function enterFull() {
    if (!mview) return;
    var v = $('.mplayer video', mview), el = v || $('.mplayer', mview);
    if (!el) { zoomEnter(); return; }
    var fn = el.requestFullscreen || el.webkitRequestFullscreen || el.msRequestFullscreen;
    /* Safari and iOS expose full screen only on the video element itself. */
    if (!fn && v && v.webkitEnterFullscreen) { try { v.webkitEnterFullscreen(); return; } catch (e) { zoomEnter(); return; } }
    if (!fn) { zoomEnter(); return; }
    var res;
    try { res = fn.call(el); } catch (e) { zoomEnter(); return; }
    armWatch();
    if (res && res.then) {
      res.then(function () { clearWatch(); setFullLabels(); })['catch'](function () {
        clearWatch();
  ...
  function toggleVideoFull() { if (fsElement() || zoomOn()) exitFull(); else enterFull(); }
  function onFsChange() {
    if (fsElement()) {
      clearWatch();
      /* real full screen won after all — drop the fallback view */
      if (zoomOn()) {
        mview.classList.remove('mzoom');
        var n = $('.mnote', mview); if (n) { n.classList.remove('on'); n.textContent = ''; }
      }
  ...
  if (mview) {
    $('.mclose', mview).addEventListener('click', function () { closeOverlays(); });
    mview.addEventListener('click', function (e) { if (e.target === mview) closeOverlays(); });
    $$('.mfull', mview).forEach(function (b) { b.addEventListener('click', toggleVideoFull); });
  ...
  function closeOverlays() {
    ...
    if (mview) { zoomExit(); $$('video', mview).forEach(function (v) { v.pause(); }); }
    hideTip(true);
  }
  ...
  barEl.addEventListener('click', function (e) {
    ...
    else if (a === 'pictures') openX('x-pictures', 0); else if (a === 'videos') openMedia(0);
  ...
  document.addEventListener('keydown', function (e) {
    ...
    var k = e.key;
    /* v69: Esc leaves the full-window video first, and only then closes the library. */
    if (k === 'Escape' && zoomOn()) { zoomExit(); return; }
    ...
    else if (k === 'i' || k === 'I') openX('x-pictures', 0);
    else if (k === 'v' || k === 'V') openMedia(0);
  ...
  stage.addEventListener('click', function (e) {
    ...
    if (vb) { openMedia(+vb.getAttribute('data-video-index') || 0); return; }
  ...
  /* Build the complete local picture library. These are the existing deck assets,
     grouped into readable pages so every image can be opened at presentation size. */
  var pictureGroups = [
    [['Traffic example','assets/figures/image7.gif'],['Public-space example','assets/figures/image6.jpeg'],['Pedestrian example','assets/figures/image8.gif'],['Drone example','assets/figures/image26.jpeg']],
    [['Traffic still image','assets/figures/image7.png'],['Tracking still image','assets/figures/image8.png'],['Aerial road reference','assets/figures/image23.jpeg'],['Aerial grayscale reference','assets/figures/image25.jpeg']],
    [['Detection boxes','assets/figures/image2.png'],['Tracked object example','assets/figures/image3.png'],['After an ID change','assets/figures/image4.png'],['Aerial tracking frame','assets/figures/image15.jpeg']],
    [['Easy scene','assets/figures/image1.png'],['Dense scene','assets/figures/image9.png'],['Difficult drone scene','assets/figures/image20.jpeg'],['Low-light scene','assets/figures/image21.jpeg']],
    [['Camera-motion example','assets/figures/image28.jpeg'],['Highway reference frame','assets/figures/image29.jpeg'],['Indoor tracking frame','assets/figures/image22.png'],['Video cover image','assets/figures/posterImage.png']],
    [['Confidence-score example','assets/figures/image5.png'],['IoU example','assets/figures/image12.png'],['False-positive example','assets/figures/image13.jpeg'],['False-negative example','assets/figures/image14.jpeg']],
    [['Small objects · easy','assets/figures/generated/small_easy.jpg'],['Edge detail · easy','assets/figures/generated/edges_easy.png'],['Brightness · day','assets/figures/generated/gray_day.jpg'],['Blur · sharp','assets/figures/generated/blur_sharp.jpg']],
    [['Small objects · hard','assets/figures/generated/small_hard.jpg'],['Edge detail · hard','assets/figures/generated/edges_hard.png'],['Brightness · night','assets/figures/generated/gray_night.jpg'],['Blur · blurred','assets/figures/generated/blur_blurred.jpg']]
  ];
  var picDeck = document.getElementById('x-pictures');
  if (picDeck) pictureGroups.forEach(function (group, gi) {
    var p = document.createElement('section'); p.className = 'xpage'; p.setAttribute('data-sec', '0'); p.setAttribute('data-title', 'Picture library ' + (gi + 1));
    p.innerHTML = '<header class="s-head"><div class="kicker">Picture library · ' + (gi + 1) + ' of ' + pictureGroups.length + '</div><h2>Project Pictures</h2></header><div class="s-body col"><div class="pic-grid"></div><div class="callout">All pictures are local presentation assets. Open Videos from the toolbar for the five video files.</div></div>';
    var g = $('.pic-grid', p); group.forEach(function (item) { var card = document.createElement('div'); card.className = 'pic-card'; card.innerHTML = '<div class="img contain"><img data-src="' + item[1] + '" alt="' + item[0] + '"></div><h3>' + item[0] + '</h3><p>' + item[1].split('/').pop() + '</p>'; g.appendChild(card); }); picDeck.appendChild(p);
  });
  if (xstage) {
  ...
  function xShow(i) {
    ...
    xpages.forEach(function (p, j) {
      var on = j === xi;
      p.style.display = on ? 'block' : 'none';
      p.classList.toggle('active', on); p.classList.toggle('past', j < xi);
    });
    fitSlide(xpages[xi]);
    loadMedia(xpages[xi]);
  ...
  function closeX() {
    ...
    xview.style.display = '';
    if (xdeck) { xdeck.classList.remove('open'); xdeck.style.display = ''; }
